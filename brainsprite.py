...
def get_html_template(template_name):
    ...
    template_path = os.path.join(
        os.path.dirname(__file__), 'data', template_name)
...
def view_stat_map(stat_map_img, bg_img='MNI152', cut_coords=None,
    ...
    if bg_img is not None and bg_img is not False :
        ...
        stat_map_img = image.resample_to_img(stat_map_img, bg_img , interpolation=resampling_interpolation)
    ...
    js_jquery = get_html_template('jquery.min.js')
    js_brainsprite = get_html_template('brainsprite.min.js')
